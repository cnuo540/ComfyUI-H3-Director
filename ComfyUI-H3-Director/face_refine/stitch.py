...
（自 ComfyUI_MiniMaxH3_Director director/face_refine/stitch.py 原样移植。）
"""
...
def stitch_faces(
    ...
    # 分块预算按「源帧与画布取大」（修复B）：贴回真正的显存大头是画布尺寸的
    # patch/mask——源 864×480≈2.4MB/帧，而 768² 画布≈6.6MB/帧，旧预算只看源帧
    # 估小了。单块上限 16 帧、预算 256MB：单次 CPU→GPU 拷贝 ~226MB→~113MB
    # （512 画布下约 50MB）。
    per_frame_mb = max(height * width, int(ch) * int(cw)) * 3 * 4 / 2 ** 20
    chunk = max(1, min(16, int(256 / max(per_frame_mb, 1e-6))))
