...
（自 ComfyUI_MiniMaxH3_Director director/face_refine/track.py 移植：
去掉 MiniMax 插件的 image_prep 依赖，检测权重定位统一走 pack.resolve_detector_path。）
"""
...
def load_detector(name: str):
    ...
    from .pack import resolve_detector_path

    path = resolve_detector_path(name)
    try:
        from ultralytics import YOLO
    except ImportError as exc:
        raise ImportError(
            "[H3导演台] 修脸需要 ultralytics。请执行: pip install ultralytics"
        ) from exc
    model = YOLO(path)
...
def track_and_crop(
    ...
    if images.ndim != 4 or images.shape[0] < 1:
        raise ValueError("[H3导演台] 修脸需要 IMAGE 视频帧 [N,H,W,C]。")
    ...
    if canvas_mode != "manual":
        ...
        # 32 的倍数（H3 VAE÷16 后 2×2 patch），自适应上限 768 / 1344。
    ...
    report = (
        f"检测 {found}/{n_frames} 帧, select={select}, "
